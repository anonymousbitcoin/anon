...
during the *configure* step of building anond:
...
Launch anond like this:

    $ anond -amqppubhashtx=amqp://127.0.0.1:5672
...
These options can also be provided in anon.conf.
...
From the perspective of anond, the local end of an AMQP link is write-only.

No information is broadcast that wasn't already received from the public
P2P network.

No authentication or authorization is done on peers that anond connects
...
At present, anond does not try to resend a notification if there was
...
Currently, anond appends an up-counting sequence number to each notification
