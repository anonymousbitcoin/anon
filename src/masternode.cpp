...
bool CMasternodeBroadcast::CheckOutpoint(int &nDos)
{
...

    {
        ...
        if (coins.vout[vin.prevout.n].nValue != Params().GetMasternodeCollateral(chainActive.Height()) * COIN)
        {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %d ANON, masternode=%s\n", Params().GetMasternodeCollateral(chainActive.Height()), vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when Params().GetMasternodeCollateral(chainActive.Height()) ANON tx got nMasternodeMinimumConfirmations
