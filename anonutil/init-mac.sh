...
if [ ! -f "$HOME/Library/Application Support/anon/anon.conf" ]; then
    echo "Creating anon.conf"
    mkdir -p "$HOME/Library/Application Support/anon/"
    echo "rpcuser=anonrpc" > ~/Library/Application\ Support/anon/anon.conf
    PASSWORD=$(cat /dev/urandom | env LC_CTYPE=C tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
    echo "rpcpassword=$PASSWORD" >> "$HOME/Library/Application Support/anon/anon.conf"
    echo "Complete!"
fi
